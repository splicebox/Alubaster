...
# Set to export all variables, including those imported from the config file:
set -o allexport   
...
--------------------------------------------------------
  ...
  $(basename $0) <SampleName>  <TophatDir>  <FastqDir> <OutputDir>

  Required parameters:
  
  <SampleName>  Sample name as apears in fastq files; i.e sample name would be ABC if fastq files are ABC_1.fastq.gz and ABC_2.fastq.gz;
...
SET=$1        # Sample ID;
THDIR=$2      # Path to tophat alignments for this sample;
...
# Initialize other vars and create dir structure
...
set +o allexport  # turn off exporting vars
